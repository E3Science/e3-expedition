...
const NAV_ITEMS = [
  ...
  ["students", "♟", "Students"], ["people", "◎", "People"],
...
export function createStudentDashboard({ root, isAdmin = false, classServers = [], missionPosition = 0, classroomStatus = {}, getStudent = () => ({}), getProgression = () => ({}), getInventory = () => ({}), getQuests = () => ({ quests: [] }), getItemDefinition = () => null, onOpenQuest = () => {}, onDeleteInventoryItem = () => false, onEnterSimulation = () => {}, onOpenSettings = () => {}, onSendChat = () => false, onRequestQuest = () => false, onAnswerStudyQuestion = () => false, onSwitchClass = () => {}, onSetMissionPosition = () => {}, onConnectClassroom = async () => {}, onSyncClassroom = async () => ({}), onLoadClassRoster = async () => ({ students: [] }), onAddClassStudent = async () => ({}), onRemoveClassStudent = async () => ({}), onLoadStudentStats = async () => ({}), onLoadAccounts = async () => ({ accounts: [] }), onUpdateAccount = async () => ({}) } = {}) {
  ...
  NAV_ITEMS.forEach(([key, glyph, label], index) => { if ((key === "students" || key === "people") && !isAdmin) return; const button = el("button", `dashboard-nav-item${index === 0 ? " active" : ""}`); button.type = "button"; button.dataset.view = key; button.innerHTML = `${icon(glyph)}<span>${label}</span>`; nav.appendChild(button); navButtons[key] = button; });
  ...
  function renderStudents() {
    ...
    wrap.innerHTML = `<div class="section-intro"><div><span class="eyebrow">Teacher controls</span><h2>Class roster</h2><p>Classroom students appear automatically after synchronization. Their verified Google sign-in email links them to E3.</p></div></div>`;
  ...
  function renderPeople() {
    ...
    wrap.innerHTML = `<div class="section-intro"><div><span class="eyebrow">Account administration</span><h2>People & access</h2><p>New Google logins appear here. Assign students to a class or promote an account to teacher access.</p></div></div>`;
    const panel = el("section", "dashboard-card people-list"); panel.innerHTML = `<p>Loading accounts…</p>`; wrap.appendChild(panel);
  ...
  function renderStudySession() {
    ...
    wrap.innerHTML = `<div class="study-session-header"><button type="button">← Learning Labs</button><div><span>${studyContext?.unit} · ${studyContext?.chapter}</span><h2>${studyContext?.sphere} Expedition</h2></div><em>Question ${studyQuestionIndex + 1}</em></div><div class="study-mastery" role="progressbar" aria-label="Study progress" aria-valuemin="0" aria-valuemax="10" aria-valuenow="${studyProgress}"><i style="width:${studyProgress * 10}%"></i></div><section class="study-question-card"><span>ARCHIVE PROMPT</span><h3></h3><div class="study-answer-grid"></div><p class="study-feedback">Fill the bar with correct answers. An incorrect answer removes three steps.</p></section>`;
    ...
    question.answers.forEach((answer, index) => { const button = el("button", "study-answer", answer); button.type = "button"; button.addEventListener("click", () => { const feedback = wrap.querySelector(".study-feedback"); wrap.querySelectorAll(".study-answer").forEach((entry) => { entry.disabled = true; }); if (index !== question.correct) { button.classList.add("wrong"); studyProgress = Math.max(0, studyProgress - 3); feedback.textContent = "Incorrect. The mastery signal dropped—next question incoming."; } else { button.classList.add("correct"); studyProgress = Math.min(10, studyProgress + 1); feedback.textContent = studyProgress >= 10 ? "Mastery achieved! Your sphere sample is being transmitted." : "Correct. The mastery signal increased."; } if (studyProgress >= 10) { onAnswerStudyQuestion(studyContext); window.setTimeout(() => { studyProgress = 0; studyQuestionIndex += 1; render("inventory"); }, 1600); return; } window.setTimeout(() => { studyQuestionIndex += 1; render("study"); }, 900); }); wrap.querySelector(".study-answer-grid").appendChild(button); });
    return wrap;
  }
