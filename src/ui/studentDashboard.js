...
export function createStudentDashboard({ root, isAdmin = false, classServers = [], missionPosition = 0, classroomStatus = {}, getStudent = () => ({}), getProgression = () => ({}), getInventory = () => ({}), getQuests = () => ({ quests: [] }), getItemDefinition = () => null, onOpenQuest = () => {}, onDeleteInventoryItem = () => false, onOpenSkills = () => {}, onEnterSimulation = () => {}, onOpenSettings = () => {}, onSendChat = () => false, onRequestQuest = () => false, onAnswerStudyQuestion = () => false, onSwitchClass = () => {}, onSetMissionPosition = () => {}, onConnectClassroom = async () => {}, onSyncClassroom = async () => ({}), onLoadClassRoster = async () => ({ students: [] }), onAddClassStudent = async () => ({}), onRemoveClassStudent = async () => ({}), onLoadStudentStats = async () => ({}), onLoadAccounts = async () => ({ accounts: [] }), onUpdateAccount = async () => ({}) } = {}) {
  ...
  function renderChat() { const wrap = el("div", "chat-page"); const transcript = el("section", "dashboard-card dashboard-chat"); transcript.innerHTML = `<div class="card-heading"><div><span class="eyebrow">Supabase Realtime</span><h2>Class chat</h2></div><span class="online-label"><i></i> Class online</span></div><div class="chat-messages"><div class="system-message">Private class messages remain available without the multiplayer world server.</div></div>`; const messages = transcript.querySelector(".chat-messages"); chatMessages.forEach((message) => { const line = el("div", `dashboard-chat-line ${message.kind || "chat"}`); line.textContent = message.text; messages.appendChild(line); }); const form = el("form", "dashboard-chat-form"); form.innerHTML = `<input aria-label="Chat message" maxlength="240" placeholder="Message your class"><button type="submit">Send</button>`; const input = form.querySelector("input"); ["keydown", "keyup", "keypress"].forEach((eventName) => input.addEventListener(eventName, (event) => event.stopPropagation())); form.addEventListener("submit", async (event) => { event.preventDefault(); const text = input.value.trim(); if (text && await onSendChat(text)) input.value = ""; }); transcript.appendChild(form); wrap.appendChild(transcript); requestAnimationFrame(() => { messages.scrollTop = messages.scrollHeight; }); return wrap; }
